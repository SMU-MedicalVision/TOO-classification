...
import time
from efficientnet_pytorch_c import EfficientNet
from datapreprocess.dataset_TOO import MyDataset_train_c, MyDataset_val_c
...
for i in range(0, 5):
    ...
    save_dir = './TOO_models_c'
    results = './TOO_results_c/fold' + str(fold) + '/'
    ...
    def loaddata(split_path, fold, batch_size, set_name, shuffle):
        ...
        image_datasets = {
            'train': MyDataset_train_c(split_path=split_path, fold=fold,
                                       clinic_path=clinic_path, transform=data_transforms['train']),
            'test': MyDataset_val_c(split_path=split_path, fold=fold,
                                    clinic_path=clinic_path, transform=data_transforms['test'])
        }
    ...
    model._fc = nn.Linear(num_ftrs + 11, num_class)
    ...
    for epoch in range(num_epochs):
        ...
        for data in dset_loaders['train']:
            inputs, labels, clinics = data
            labels = label_binarize(labels, classes=[i for i in range(num_class)])
            labels = torch.from_numpy(labels)
            labels = torch.squeeze(labels.type(torch.LongTensor))
            if use_gpu:
                inputs, labels, clinics = Variable(inputs.cuda()), Variable(labels.cuda()), Variable(clinics.cuda())
            else:
